...
int nAnonymizeThreeAcesAmount = 1000;
...
#else
    const char* pszModule = "ThreeAces";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\ThreeAces
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\ThreeAces
    // Mac: ~/Library/Application Support/ThreeAces
    // Unix: ~/.ThreeAces
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ThreeAces";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "ThreeAces.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "acesd.pid"));
