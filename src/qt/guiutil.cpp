...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("ThreeAces"))
        return false;
...
{
    // Convert ThreeAces:// to ThreeAces:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("ThreeAces://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "ThreeAces:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "ThreeAces.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for ThreeAces.lnk
...
{
    return GetAutostartDir() / "ThreeAces.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a ThreeAces.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=ThreeAces\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the ThreeAces app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add ThreeAces app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("ThreeAces-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  ThreeAces-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("ThreeAces-Qt"));
