...
void Shutdown()
{
    ...
    RenameThread("ThreeAces-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: ThreeAces.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: acesd.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeacesamount=<n> " + _("Keep N ThreeAces anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. ThreeAces is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. ThreeAces is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("ThreeAces version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "ThreeAces server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of ThreeAces") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart ThreeAces to complete") << "\n";
    ...
    nAnonymizeThreeAcesAmount = GetArg("-anonymizeacesamount", 0);
    if(nAnonymizeThreeAcesAmount > 999999) nAnonymizeThreeAcesAmount = 999999;
    if(nAnonymizeThreeAcesAmount < 2) nAnonymizeThreeAcesAmount = 2;
    ...
    LogPrintf("Anonymize ThreeAces Amount %d\n", nAnonymizeThreeAcesAmount);
